    ...
    if ($_POST && isset($_POST["functionName"])) {
        ...
        switch ($functionName) {
            case "join_game": // user presses "Join Game" button
                ...
                break;
            case "onStart":
                // $_SESSION["word"] == $toDump[0] should be the selected word
                // $toDump[1] should be an array with username as the key and username's points as the value
            ...
            case "input_definition": // user submits a definition
                ...
                break;
            case "onChoices":
                $toDump = $game_action->onChoices($_SESSION["gameID"]); // $toDump should be an array of string definitions
            ...
            case "select_definition": // user votes a user's definition
                $selectionID = $_SESSION["idDefs"][$_POST["functionName"][1]];
            ...
            case "onSummary":
                ...
                break;
            case "reset_round":
                $game_action->reset_round($_SESSION["gameID"]);
                break;
            case "onEnd":
                $toDump = $game_action->onEnd($_SESSION["gameID"], $_SESSION["userID"]);
                break;
        }
